...
public class V8__add_session_application_visibility extends BaseJavaMigration {

    @Override
    public void migrate(Context context) throws Exception {
        Connection connection = context.getConnection();
        String tableName = findTable(connection, "session_applications");
        if (tableName == null || findColumn(connection, tableName, "is_public") != null) {
            return;
        }
        ...
        try (Statement statement = connection.createStatement()) {
            statement.execute(
                    "ALTER TABLE " + quote(tableName, mysql)
                            + " ADD COLUMN " + quote("is_public", mysql)
