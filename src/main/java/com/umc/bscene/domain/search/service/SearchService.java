...
public class SearchService {
    ...
    public ExploreSearchResponse search(
            ...
            Genre genre, Region region, boolean withDummy, String cursor, int size
    ) {
        ...
        try {
            ExploreSearchResponse response = switch (type) {
                case ALL -> searchAll(userId, trimmed, sort, genre, region, withDummy);
                case BAND -> searchBandsOnly(userId, trimmed, sort, genre, region, withDummy, cursor, size);
                case PERFORMANCE -> searchPerformancesOnly(trimmed, sort, genre, region, withDummy, cursor, size);
                case POST -> searchPostsOnly(trimmed, sort, genre, region, withDummy, cursor, size);
            };
    ...
    private ExploreSearchResponse searchAll(Long userId, String keyword, SearchSortType sort, Genre genre, Region region, boolean withDummy) {
        NativeQuery bandQuery = buildQuery(keyword, BAND_FUZZY_FIELDS, BAND_EXACT_FIELDS, BAND_PREFIX_FIELDS,
                "name", genre, region, withDummy, "createdAt", sort, SECTION_SIZE, null);
        NativeQuery performanceQuery = buildQuery(keyword, PERFORMANCE_FUZZY_FIELDS, PERFORMANCE_EXACT_FIELDS, PERFORMANCE_PREFIX_FIELDS,
                "title", genre, region, withDummy, "performanceDate", sort, SECTION_SIZE, null);
        NativeQuery postQuery = buildQuery(keyword, POST_FUZZY_FIELDS, POST_EXACT_FIELDS, POST_PREFIX_FIELDS,
                "title", genre, region, withDummy, "uploadedAt", sort, SECTION_SIZE, null);
    ...
    private ExploreSearchResponse searchBandsOnly(Long userId, String keyword, SearchSortType sort, Genre genre, Region region, boolean withDummy, String cursor, int size) {
        int pageSize = clampSize(size);
        NativeQuery query = buildQuery(keyword, BAND_FUZZY_FIELDS, BAND_EXACT_FIELDS, BAND_PREFIX_FIELDS,
                "name", genre, region, withDummy, "createdAt", sort, pageSize + 1, SearchCursor.decode(cursor, sort));
    ...
    private ExploreSearchResponse searchPerformancesOnly(String keyword, SearchSortType sort, Genre genre, Region region, boolean withDummy, String cursor, int size) {
        int pageSize = clampSize(size);
        NativeQuery query = buildQuery(keyword, PERFORMANCE_FUZZY_FIELDS, PERFORMANCE_EXACT_FIELDS, PERFORMANCE_PREFIX_FIELDS,
                "title", genre, region, withDummy, "performanceDate", sort, pageSize + 1, SearchCursor.decode(cursor, sort));
    ...
    private ExploreSearchResponse searchPostsOnly(String keyword, SearchSortType sort, Genre genre, Region region, boolean withDummy, String cursor, int size) {
        int pageSize = clampSize(size);
        NativeQuery query = buildQuery(keyword, POST_FUZZY_FIELDS, POST_EXACT_FIELDS, POST_PREFIX_FIELDS,
                "title", genre, region, withDummy, "uploadedAt", sort, pageSize + 1, SearchCursor.decode(cursor, sort));
     ...
     * filter : 장르·지역 선택 시 + withDummy=false 시 더미 밴드 경계 초과 id만 (점수 무관, 캐싱)
    ...
    private NativeQuery buildQuery(
            ...
            String titleField, Genre genre, Region region, boolean withDummy, String dateSortField, SearchSortType sort,
        ...
        var builder = NativeQuery.builder()
                .withQuery(q -> q.bool(b -> {
                    ...
                    // withDummy=false : 더미 밴드 제외 (경계 초과 id만) — 세 인덱스 모두 bandId 필드를 가져
